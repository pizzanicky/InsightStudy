...
def send_report_email(to_email: str, subject: str, summary_md: str, cover_card: dict = None, ticker: str = "", date_str: str = None) -> dict:
    ...
    try:
        ...
        # 5. Ensure proper spacing for References section
        summary_md = re.sub(r'(?i)([^\n])\s*\*\*(参考文献|References)[：:]\*\*', r'\1\n\n**\2：**', summary_md)
        summary_md = re.sub(r'(?i)(\n)\s*\*\*(参考文献|References)[：:]\*\*', r'\n\n**\2：**', summary_md)
...
def _create_email_html(content_html: str, card: dict, ticker: str, date_str: str = None) -> str:
    ...
    if card:
        ...
        # Light Theme Badge Colors (Text/BG) - High contrast
        if score >= 6:
            badge_color = "#047857" # Dark Emerald Green Text
            badge_bg = "#d1fae5"    # Light Emerald BG
            badge_border = "#a7f3d0"
        elif score <= 4:
            badge_color = "#b91c1c" # Dark Red Text
            badge_bg = "#fee2e2"    # Light Red BG
            badge_border = "#fecaca"
        else:
            badge_color = "#b45309" # Dark Amber Text
            badge_bg = "#fef3c7"    # Light Amber BG
            badge_border = "#fde68a"
        ...
        # Factors Tags: Light grey background, dark text
        factors_html = "".join([
            f'<span style="display:inline-block; background:#f1f5f9; color:#334155; padding:4px 10px; border-radius:10px; font-size:12px; margin:2px; border:1px solid #cbd5e1;">{f}</span>' 
            for f in factors
        ])
